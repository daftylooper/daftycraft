...
void PerlinNoise::generatePerlin(){
    ...
    std::cout<<"Size of Perlin Buffer nxn : "<<pixels.size()<<std::endl;
}

void PerlinNoise::generateCubes(){
    for(int i=0; i<PerlinNoise::pixels.size(); i++){
        for(int j=0; j<PerlinNoise::pixels[i].size(); j++){
            for(int k=0; k<PerlinNoise::pixels[i][j]; k++){
                cubes.push_back(glm::vec3(0.0f+i*1.0f, 0.0f+k*1.0f, 0.0f+j*1.0f));
            }
            // std::cout<<PerlinNoise::pixels[i][j]<<" ";
        }
    }
}
